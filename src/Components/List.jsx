  ...
  return (
    <>
      <div className=" w-full mx-5 sm:w-9/12 flex flex-col  mt-10 rounded-t">
        ...
        <div className="h-full flex gap-1 flex-col bg-transparent mt-1 overflow-y-hidden overflow-x-visible" style={{ 
            scrollbarWidth: 'none',
            scrollbarColor: 'none none'
        }}>
          {allLinks?.slice()?.reverse()?.map((value, index) => {
            return (
              <motion.div
              ...
              className="flex w-full justify-center md:justify-between text-center text-base font-normal text-[#C9CED6] p-2 bg-[rgba(24, 30, 41, 0.80)] backdrop-blur-2xl shadow-xl hover:border-solid">
                ...
                <div  className="w-full md:w-4/12 flex flex-row items-center justify-center me-3"><a
                 ref={linkRef}
                href={"https://theslugproject.onrender.com/"+value.shortURL} target="_blank" style={{ whiteSpace: "nowrap" }} className=" ms-1 overflow-hidden">{"localhost:10000/"+value.shortURL}</a>
                <button  onClick={copyToClipboard} className="ms-2  bg-[rgba(28, 40, 63, 0.69)] p-1 rounded-full "><IoIosCopy size={20} color="" /></button></div>
                <div className="w-2/12 hidden md:flex flex-row items-center">
                  <img
                    ...
                    alt="logo"
                  />
                  <p target="_blank" style={{ whiteSpace: "nowrap" }} className=" ms-1 overflow-hidden">
                    {value.redirectURL}
                  </p>
                </div>
                <div className="w-2/12 hidden md:flex items-center justify-center">
                  ...
                  <img
                    className=" ms-2 w-10 h-10"
                    src={
                      "https://theslugproject.onrender.com/public/assets/url_qrs/"+value.url_qr
                    }
                    alt="Qr code"
                  />
                 
                </div>
                <p className="w-2/12 hidden md:flex items-center justify-center">{value.clicks}</p>
                <p className="w-2/12 hidden md:flex items-center justify-center">Active</p>
